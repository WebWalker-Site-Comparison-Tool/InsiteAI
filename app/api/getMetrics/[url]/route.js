...
export async function GET(req) {
  ...
  if (cacheExist === false) {
    lighthouseOutput = await axios.get(
      'http://localhost:4000/lighthouse?url=' + urlPlain
    ); //need try catch logic
  ...
  if (cacheExist === false) {
    ...
    try {
      chatGPTOutput = await axios.put('http://127.0.0.1:3000/api/chatgpt', {
        prompt
      });
      chatGPTOutput = chatGPTOutput.data;
      console.log(
        'type of chatgpt getmetrics ISARRAY',
        Array.isArray(chatGPTOutput)
      );
  ...
  if (cacheExist) {
    ...
    let storedOutput = Object.assign(
